...
$Gorod = filter_var(trim($_POST['AddGorod']), FILTER_SANITIZE_STRING);
$Dos = filter_var(trim($_POST['AddDos']), FILTER_SANITIZE_STRING);
$TypeDos= filter_var(trim($_POST['AddTypeDos']), FILTER_SANITIZE_STRING);
$Cafe = filter_var(trim($_POST['AddCafe']), FILTER_SANITIZE_STRING);
$CheckCafe = filter_var(trim($_POST['AddCheckCafe']), FILTER_SANITIZE_STRING);


if (mb_strlen($Gorod) < 3 || mb_strlen($Gorod) > 90) {
  echo"<script>swal(\"Недопустимая длина города!\", \"Город содержит от 3 до 90 символов\", \"error\");</script>";
    exit();
} elseif (mb_strlen($Dos) < 3 || mb_strlen($Dos) > 90) {
    echo"<script>swal(\"Недопустимая длина достопримечательности!\", \"Достопримечательность содержит от 3 до 90 символов\", \"error\");</script>";
    exit();
} elseif (mb_strlen($TypeDos) < 3 || mb_strlen($TypeDos) > 90) {
    echo"<script>swal(\"Недопустимая длина типа достопримечательности!\", \"Тип достопримечательности содержит от 3 до 90 символов\", \"error\");</script>";
    exit();
}elseif (mb_strlen($Cafe) < 3 || mb_strlen($Cafe) > 90) {
    echo"<script>swal(\"Недопустимая длина кафе!\", \"Кафе содержит от 3 до 90 символов\", \"error\");</script>";
    exit();
}
elseif (mb_strlen($CheckCafe) < 3 || mb_strlen($CheckCafe) > 90) {
    echo"<script>swal(\"Недопустимая длина среднего чека!\", \"Средний чек содержит от 3 до 90 символов\", \"error\");</script>";
    exit();
}
...
$mysql->query("INSERT INTO `infa` (`gorod` , `dos` , `typeDos`,`cafe`,`checkCafe`)
VALUES('$Gorod','$Dos','$TypeDos','$Cafe','$CheckCafe')");
$mysql->close();
echo"<script>swal(\"Успешно!\", \"Информация успешно добавлена\", \"success\");</script>";
