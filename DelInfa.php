...
$user = $result->fetch_assoc();
if(count($user)==0){
  echo"<script>swal(\"Такой информации нет!\", \"Поменяйте № Строки\", \"error\");</script>";
}else{
  $mysql->query("DELETE FROM infa WHERE id = '$id'");
  $mysql->close();
  echo"<script>swal(\"Успешно!\", \"Информация успешно удалена\", \"success\");</script>";
  exit();
}
